...
def getClusters(all_grades, student_final_grade):
    ...
    c1 = [student_final_grade.get(key) for key in student_final_grade if
          student_final_grade.get(key) <= 0.25 * avg_value]
    c2 = [student_final_grade.get(key) for key in student_final_grade if
          0.25 * avg_value < student_final_grade.get(key) <= 0.50 * avg_value]
    c3 = [student_final_grade.get(key) for key in student_final_grade if
          0.50 * avg_value < student_final_grade.get(key) <= 0.75 * avg_value]
    c4 = [student_final_grade.get(key) for key in student_final_grade if
          student_final_grade.get(key) > 0.75 * avg_value]
...
def drawFinalBoxplot(d):
    ...
    all_final_grades_without_suggestions = []
    student_final_grade ={}

    for key in d:
        final_grade = [float(row[10]) for row in file if row[1] == str(key)]
        all_final_grades_without_suggestions.append(final_grade[0])
        student_final_grade[key] = final_grade[0]


    c1,c2,c3,c4 = getClusters(all_final_grades_without_suggestions, student_final_grade)


    drawBoxPlot(c1,c2,c3,c4,"before")


    new_student_final_grade={}
    all_final_grades_with_suggestions=[]
    print(student_final_grade)
    print(d)
    for k in student_final_grade:

        if d.get(k)[1] == 0:
            expected_final_grade = student_final_grade.get(k) + 300
        else:
            expected_final_grade = student_final_grade.get(k) - d.get(k)[0] + d.get(k)[1]

        new_student_final_grade[k] = expected_final_grade
        all_final_grades_with_suggestions.append(expected_final_grade)

    c1_1, c2_2, c3_3, c4_4 = getClusters(all_final_grades_with_suggestions, new_student_final_grade)
    drawBoxPlot(c1_1, c2_2, c3_3, c4_4, "after")
